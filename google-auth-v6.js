...
(function(){
  ...
  function loadGIS(){return new Promise((resolve,reject)=>{if(window.google?.accounts?.id)return resolve();let s=document.querySelector('script[data-cb-gis]');if(s){s.addEventListener('load',resolve,{once:true});s.addEventListener('error',reject,{once:true});return;}s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.async=true;s.dataset.cbGis='1';s.onload=resolve;s.onerror=reject;document.head.appendChild(s);});}
  function closeModal(){document.getElementById('cbGoogleModal')?.remove();}
  function fallbackLogin(message){
    closeModal();
    toast(message||'Opening secure sign-in…');
    setTimeout(()=>legacyGoogleLogin(),650);
  }
  ...
  async function finishGoogle(response){
    ...
    }catch(e){console.error('Car Bestie Google token sign-in failed',e);fallbackLogin('Trying secure fallback…');}
  }
  window.googleLogin=async function(){
    ...
    try{
      ...
      google.accounts.id.initialize({client_id:GCLIENT,callback:finishGoogle,nonce:hashedNonce,ux_mode:'popup',use_fedcm_for_prompt:true,context:'signin'});
    ...
    }catch(e){console.error('Car Bestie Google Identity unavailable',e);fallbackLogin('Google popup unavailable — using secure fallback…');}
  };
})();
